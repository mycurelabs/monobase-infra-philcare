...
# Examples:
argocd app sync <client>-staging-root
argocd app sync <client>-production-root --prune
```
...
# Examples:
argocd app diff <client>-staging-root
...
# Examples:
argocd app list
argocd app get <client>-staging-root
```
...
# Examples:
argocd app history <client>-staging-root
argocd app rollback <client>-staging-root 5
```
...
# Examples:
kubectl patch application <client>-staging-root -n argocd --type merge -p '{"spec":{"syncPolicy":null}}'
```
...
# Examples:
argocd app get <client>-staging-root --hard-refresh
```
...
The `monobase-auto-discover` ApplicationSet (in `charts/argocd-bootstrap/templates/applicationset-auto-discover.yaml`) uses a **Git Files Generator** to scan `values/deployments/*.yaml`:

1. Scans the flat `values/deployments/*.yaml` (excludes `example-*.yaml`; files with `disabled: "true"` are filtered out by the selector)
2. For each file (e.g., `<client>-production.yaml`):
   - Creates a root Application named `{filename}-root` (e.g., `<client>-production-root`)
   - Deploys to namespace matching filename (e.g., `<client>-production`)
   - Uses the YAML file as Helm values
3. Each root Application renders `charts/argocd-applications/` templates → deploys full stack

## Two-Level App-of-Apps Pattern

```
ApplicationSet (monobase-auto-discover)
  └── Per-deployment root Application (e.g., <client>-production-root)
        ├── namespace chart
        ├── hapihub chart
        ├── app chart (per enabled app)
        └── ... (all enabled charts)
...
# Manual bootstrap steps (the argocd-bootstrap chart renders both):
# 1. Install ArgoCD
# 2. helm template + apply charts/argocd-bootstrap (infrastructure-root.yaml)
# 3. helm template + apply charts/argocd-bootstrap (applicationset-auto-discover.yaml)
```

## Infrastructure vs Deployment Values

- `values/clusters/<cluster>/argocd/infrastructure.yaml` — Cluster-wide infrastructure config (cert-manager, gateway, monitoring, etc.)
...
```bash
APP=<app-name>         # e.g. <client>-staging-hapihub
NS=<target-namespace>  # e.g. <client>-staging
...
| User says | Do |
|---|---|
| "is <client>-staging healthy?" | `kubectl -n argocd get app ...` grouped by Health first, Sync second |
