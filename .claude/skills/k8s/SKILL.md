...
# Examples:
kubectl logs -f -n <client>-staging deployment/api --all-containers --prefix
kubectl logs -n <client>-production deployment/hapihub --tail=100
```
...
# Examples:
kubectl rollout restart deployment/api -n <client>-staging
kubectl rollout status deployment/api -n <client>-staging
```
...
# Examples:
kubectl describe pod api-7d8f9b6c4-x2k3l -n <client>-staging
kubectl logs api-7d8f9b6c4-x2k3l -n <client>-staging --tail=100
kubectl get events -n <client>-staging --sort-by=.lastTimestamp | grep api
```
...
# Examples:
kubectl exec -it api-7d8f9b6c4-x2k3l -n <client>-staging -- /bin/sh
kubectl exec api-7d8f9b6c4-x2k3l -n <client>-staging -- env
```
...
# Examples:
kubectl get events -n <client>-staging --sort-by=.lastTimestamp
...
# Examples:
kubectl scale deployment/api -n <client>-staging --replicas=3
kubectl scale deployment/hapihub -n <client>-staging --replicas=0  # ⚠️ STOPS SERVICE
```
...
# Examples:
kubectl exec -it -n <client>-staging postgresql-0 -- psql -U postgres
```
...
# Examples:
kubectl annotate externalsecret api-secrets -n <client>-staging force-sync=$(date +%s) --overwrite
```
...
# Examples:
kubectl get externalsecret -n <client>-staging -o wide
kubectl describe externalsecret api-secrets -n <client>-staging
```
...
- Tenant namespaces: `{client}-{environment}` (e.g., `<client>-production`, `<client>-staging`)
...
- Internal gateway: `nginx-internal-gateway` — cluster-default ingress, ClusterIP data plane exposed on the private network
- Gateway class: `nginx` (NGINX Gateway Fabric)
- Multi-domain listeners: e.g. `*.example.com`, `*.staging.example.com`, `*.example.org` — configured per cluster in `values/`
- HTTPRoutes in each tenant namespace reference the gateways via `parentRefs`
- Cluster-wide nginx tweaks (timeouts, body size, header filters) via SnippetsPolicies in `values/clusters/<cluster>/argocd/infrastructure.yaml`
...
2. **App unreachable?** → Check outside-in:
   - Gateway: `kubectl get gateway -n nginx-gateway-system`
