...
kubectl describe certificate gateway-tls -n nginx-gateway-system
```
...
kubectl get gateway nginx-shared-gateway -n nginx-gateway-system -o yaml | grep -A3 "listeners:"

# Common issue: sectionName mismatch
# Production *.example.com uses: sectionName: https-public
# Staging *.staging.example.com uses: sectionName: https-staging
```
...
gcloud secrets list --project=<gcp-project> --filter="name:{remote-key}"
```
