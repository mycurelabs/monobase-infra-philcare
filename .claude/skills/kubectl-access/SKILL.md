...
This skill tells Claude how to obtain and use cluster credentials when working in the `infra` repo. It must be applied **before any `kubectl` invocation**.
...
1. `.kube/config`
2. `.kubeconfig` (repo root)
3. `kubeconfig.yaml` or any `*.kubeconfig` in the repo root
4. **Fallback only if nothing local exists:** `~/.kube/config` and per-cluster files in `~/.kube/` (this repo's docs install kubeconfigs as `~/.kube/<cluster-name>`, e.g. `~/.kube/aws-main`)
