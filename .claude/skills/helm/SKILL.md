---
name: helm
description: Helm chart management for the repo's charts
...
3. **Default:** `~/.kube/aws-main` (if exists)
...
```bash
export KUBECONFIG=~/.kube/aws-main
```
...
# Examples:
helm diff upgrade app charts/app -f values/deployments/<client>-staging.yaml -n <client>-staging
helm diff upgrade hapihub charts/hapihub -f values/deployments/<client>-production.yaml -n <client>-production
```
...
# Examples:
helm get values app -n <client>-staging
helm get values hapihub -n <client>-production -a -o yaml
```
...
# Examples:
helm template app charts/app -f values/deployments/<client>-staging.yaml --debug
helm template hapihub charts/hapihub -f values/deployments/<client>-production.yaml --validate
```
...
helm lint charts/hapihub -f values/deployments/<client>-staging.yaml
```
...
# Examples:
helm history app -n <client>-staging
helm history hapihub -n <client>-production
```
...
See `references/chart-inventory.md` for the full list. High-level groups:

### Applications

- `hapihub` — HapiHub API (Bun/PostgreSQL, healthcare backend; reference chart)
- `app` — generic frontend/service chart (instantiate per app instead of cloning)
- `medley`, `medgemma-worker`, `openmed` — medical AI services (Hono/Bun)
- `nocodb` — no-code DB UI over Postgres

### Data-plane / Sync

- `cadence` — cadence P2P sync hub (iroh)
- `cadence-relay` — self-hosted iroh relay

### Infrastructure

- `argocd-bootstrap`, `argocd-applications`, `argocd-infrastructure` — GitOps wiring
- `namespace` — namespace + security + resource quotas
- `nginx-gateway` — shared Gateways (public + internal)
- `cert-manager-issuers` — multi-provider ClusterIssuers
- `coredns-custom` — managed-CoreDNS override hook (in-cluster DNS rewrites)
- `external-dns` — automatic DNS record management
- `database-secrets`, `external-secrets-stores` — External Secrets wiring
- `tailscale-operator`, `ollama-egress` — private-network exposure / egress
- `minio-httproute`, `minio-artifacts-httproute` — MinIO routing

### Storage / Backup

- `velero-resources` — backup locations, snapshot locations, schedules

### Observability

- `grafana`, `signoz` — dashboards / APM with Gateway API integration
- `monitoring-resources` — PrometheusRules, ServiceMonitors, dashboards, Alertmanager config

### Security

- `security-baseline` — NetworkPolicies and RBAC
- `kyverno-resources` — Kyverno ClusterPolicies
- `falco-resources` — Falco runtime detection rules
- `tenzir` — security detection pipeline
...
global:
  ...
  namespace: <client>-production         # Target namespace
  environment: production              # Environment name
  nodePool: "production"               # Node affinity pool
  gateway:
    name: nginx-shared-gateway         # Gateway resource name (or nginx-internal-gateway)
...
parentRefs:
  - name: {{ include "{chart}.gateway.name" . }}
    ...
    sectionName: https-public  # Listener for *.example.com
```
