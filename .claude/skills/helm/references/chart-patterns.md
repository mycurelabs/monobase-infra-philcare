...
{{- default "nginx-shared-gateway" .Values.global.gateway.name }}
{{- end }}
...
- `https-public` — Production `*.example.com` listener
- `https-staging` — Staging `*.staging.example.com` listener
- (omit) — Default `*.example.com` listener
