...
| `kubernetes_version` | string | (required) | Full version slug (`doctl kubernetes options versions`) |
| `node_pools` | map(object) | (required) | Explicit node-pool definitions (size, count, autoscale, taints, labels) |
| `default_node_pool_key` | string | (required) | Which `node_pools` key is the DOKS default pool |
| `ha_control_plane` | bool | `false` | Highly-available control plane |
| `auto_upgrade` | bool | `true` | DO-managed patch auto-upgrade |
| `surge_upgrade` | bool | `true` | Surge upgrades |
| `vpc_cidr` | string | `"10.244.0.0/16"` | VPC CIDR |
| `tags` | list(string) | `[]` | Cluster tags |
| `maintenance_window_day` | string | `"sunday"` | Maintenance window day |
| `maintenance_window_hour` | string | `"04:00"` | Maintenance window start |

> This module takes an explicit `node_pools` map rather than a `deployment_profile` preset.
> Size the pools directly in the cluster root's `terraform.tfvars`.

### Key Outputs

- `cluster_id`, `cluster_name`, `cluster_endpoint`, `cluster_version`
- `cluster_urn`, `cluster_ipv4_address`, `cluster_status`
- `vpc_id`, `vpc_cidr`, `vpc_urn`, `node_pool_ids`
- `kubeconfig` / `raw_kubeconfig` (sensitive), `cluster_ca_certificate`, `configure_kubectl`
