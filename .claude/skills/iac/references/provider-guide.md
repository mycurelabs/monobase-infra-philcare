...
Each cloud provider requires static IPs for the gateway LoadBalancer. Provider annotations go under `nginxGatewayResources.gateway.infrastructure.annotations` in `values/clusters/<cluster>/argocd/infrastructure.yaml` — NGF propagates them onto the provisioned Service. See `docs/infrastructure/static-ip-prerequisites.md`.
...
- **Config**: annotation `service.beta.kubernetes.io/do-loadbalancer-name` (LB is REGIONAL_NETWORK, updates in place)
...
*Currently all environments use GCP Secret Manager (project configured per cluster) via ClusterSecretStore `gcp-secretstore`.
