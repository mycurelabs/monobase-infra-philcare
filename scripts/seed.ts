#!/usr/bin/env bun
/**
 * Seed script for deployment environments
...
const ENVS: Record<string, { api: string; cms: string }> = {
  preprod: {
    api: "https://hapihub.preprod.example.com",
    cms: "https://mycure.preprod.example.com",
  },
  production: {
    api: "https://hapihub.example.com",
    cms: "https://mycure.example.com",
  },
};

function printUsage() {
  console.log(`
${chalk.bold("Seed Script")}
Creates a demo organization with 7 role-based user accounts.
...
${chalk.yellow("Options:")}
  ...
  --reset     Delete all existing seed data before re-seeding (idempotent
              ...
              when superadmin@example.test was created. If the password
  ...
  --patient-accounts
              ...
              account=<uid>. Passwords are the same ChangeMe123! used for
  ...
  Always-on settings seeded into ALL 3 facilities (idempotent, --reset wipes):
    ...
    EMR:
      - Form Templates: ~30 starter presets vendored from the product
    ...
    PME:
      - Report Templates: 5 starter presets vendored from the product
...
// per env in values/deployments/<client>-{preprod,production}.yaml) BEFORE
...
const SERVICE_ACCOUNT_EMAILS = new Set<string>(["service@example.com"]);
...
const USERS: SeedUser[] = [
  ...
  // isServiceAccount().
  { email: "service@example.com",      name: "Platform Service Account", roleIds: [],                                                  superadmin: false },
  ...
  { email: "superadmin@example.test", name: "Carlos Tan",      roleIds: ["admin", "clinic_manager"],                        superadmin: true  },
  { email: "admin@example.test",      name: "Beatriz Lim",     roleIds: ["admin", "clinic_manager"],                        superadmin: false },
  ...
  { email: "doctor@example.test",     name: "Juan Cruz",       roleIds: ["doctor", "doctor_pme", "medical_head"],            superadmin: false },
  { email: "pedia@example.test",      name: "Sofia Reyes",     roleIds: ["doctor"],                                          superadmin: false },
  { email: "familymd@example.test",   name: "Mateo Santos",    roleIds: ["doctor"],                                          superadmin: false },
  { email: "nurse@example.test",      name: "Maria Santos",    roleIds: ["nurse", "nurse_head", "frontdesk"],                superadmin: false },
  { email: "cashier@example.test",    name: "Ana Reyes",       roleIds: ["billing", "billing_encoder", "frontdesk"],         superadmin: false },
  // Dedicated receptionist (primary role 'frontdesk' = "Receptionist").
  { email: "reception@example.test",  name: "Liza Mendoza",    roleIds: ["frontdesk"],                                       superadmin: false },
  { email: "laboratory@example.test", name: "Pedro Bautista",  roleIds: ["med_tech", "lab_tech", "lab_qc"],                  superadmin: false },
  { email: "imaging@example.test",    name: "Rosa Villanueva", roleIds: ["radiologic_tech", "imaging_tech", "imaging_qc"],   superadmin: false },
  ...
  { email: "pharmacist@example.test", name: "Grace Tolentino", roleIds: ["pharmacist", "pharmacy_custodian"],                superadmin: false },
];
...
const USER_PROFILES: Record<string, UserProfileExtras> = {
  ...
  "doctor@example.test": {
    ...
    doc_bio:
      ...
      "Subspecialty in adult endocrinology. Active consultant at Example Demo Clinic.",
...
async function createOrganization(
  ...
  return (await api("POST", "/organizations", {
    ...
    description: parent
      ? "Example demo branch (child facility for hierarchy testing)"
      : "Example demo clinic for environment verification",
    ...(parent ? { parent } : {}),
  })) as { id?: string };
}
...
interface OrgProfile {
  ...
  name: string;
  /** "Example Demo Clinic" is the parent; the rest are child branches. */
  parentName?: string;
  description: string;
  /**
...
const ORG_PROFILES: OrgProfile[] = [
  ...
  {
    name: "Example Demo Clinic",
    description:
      "Flagship outpatient clinic of the Example demo network. Multi-specialty " +
    ...
    types: ["clinic"],
    email: "main@example-demo.example.ph",
    phone: "+6328123450",
    ...
    socialMediaURLs: {
      facebook: "https://facebook.com/example-demo",
      instagram: "https://instagram.com/example.demo",
      twitter: "https://twitter.com/example_demo",
    },
    tags: ["demo", "seed", "flagship"],
  },
  // Branch 1 — Quezon City
  {
    name: "Example Demo Branch - QC",
    parentName: "Example Demo Clinic",
    description:
      "Example Demo Clinic — Quezon City branch. Outpatient services and walk-in PME clinic.",
    types: ["clinic"],
    email: "qc@example-demo.example.ph",
    phone: "+6329123451",
    ...
    socialMediaURLs: {
      facebook: "https://facebook.com/example-demo-qc",
    },
    tags: ["demo", "seed", "branch"],
  },
  // Branch 2 — Cebu
  {
    name: "Example Demo Branch - Cebu",
    parentName: "Example Demo Clinic",
    description:
      "Example Demo Clinic — Cebu City branch. Provincial outpatient and diagnostic centre.",
    types: ["clinic"],
    email: "cebu@example-demo.example.ph",
    phone: "+6332123452",
    ...
    socialMediaURLs: {
      facebook: "https://facebook.com/example-demo-cebu",
    },
...
function generatePatient(facility: string, externalId: string): SeedPatient {
  ...
  // Email = first.last+ext@example.test for uniqueness across reruns
...
//
// All records use Dr. Juan Cruz (doctor@example.test) as the provider.
...
const FIXED_PATIENT_DOCTOR_EMAIL = "doctor@example.test";
  ...
  try {
    const created = (await api("POST", "/medical-patients", {
      ...
      personalDetails: {
        ...
        email: "pedro.demo.lopez@example.test",
        bloodType: "O+",
        maritalStatus: "married",
        nationality: "Filipino",
...
// FORM_TEMPLATE_PRESETS names (apps/<app>/src/pages/emr/formTemplatePresets.ts).
...
const SEED_PROVIDER_ASSIGNMENTS: ProviderAssignment[] = [
  ...
  { userEmail: "doctor@example.test",   serviceName: "General Consultation",         serviceType: "clinical-consultation", readersFeePercentage: 70 },
  { userEmail: "doctor@example.test",   serviceName: "Follow-up Consultation",       serviceType: "clinical-consultation", readersFeePercentage: 70 },
  { userEmail: "doctor@example.test",   serviceName: "Specialist Consultation",      serviceType: "clinical-consultation", readersFeePercentage: 80 },
  // Pediatrician (Dr. Reyes) — general consults + telemedicine
  { userEmail: "pedia@example.test",    serviceName: "General Consultation",         serviceType: "clinical-consultation", readersFeePercentage: 70 },
  { userEmail: "pedia@example.test",    serviceName: "Follow-up Consultation",       serviceType: "clinical-consultation", readersFeePercentage: 70 },
  { userEmail: "pedia@example.test",    serviceName: "Telemedicine Consultation",    serviceType: "clinical-consultation", readersFeePercentage: 75 },
  // Family medicine (Dr. Santos) — general + telemed + specialist
  { userEmail: "familymd@example.test", serviceName: "General Consultation",         serviceType: "clinical-consultation", readersFeePercentage: 70 },
  { userEmail: "familymd@example.test", serviceName: "Specialist Consultation",      serviceType: "clinical-consultation", readersFeePercentage: 80 },
  { userEmail: "familymd@example.test", serviceName: "Telemedicine Consultation",    serviceType: "clinical-consultation", readersFeePercentage: 75 },
  // Nurse procedures (small commission for procedural support)
  { userEmail: "nurse@example.test",    serviceName: "Nebulization",                 serviceType: "clinical-procedure",    readersFeePercentage: 20 },
  { userEmail: "nurse@example.test",    serviceName: "IV Insertion / IVF Drip",      serviceType: "clinical-procedure",    readersFeePercentage: 20 },
  // Imaging tech reads X-ray / ECG / UTZ
  { userEmail: "imaging@example.test",  serviceName: "Chest X-ray (PA view)",        serviceType: "diagnostic",            readersFeePercentage: 40 },
  { userEmail: "imaging@example.test",  serviceName: "Chest X-ray (PA + Lateral)",   serviceType: "diagnostic",            readersFeePercentage: 40 },
  { userEmail: "imaging@example.test",  serviceName: "12-lead ECG",                  serviceType: "diagnostic",            readersFeePercentage: 40 },
  { userEmail: "imaging@example.test",  serviceName: "Abdominal Ultrasound",         serviceType: "diagnostic",            readersFeePercentage: 50 },
  { userEmail: "imaging@example.test",  serviceName: "Obstetric Ultrasound",         serviceType: "diagnostic",            readersFeePercentage: 50 },
];
...
const SEED_WITHHOLDING_TAXES: WithholdingTaxAssignment[] = [
  { userEmail: "doctor@example.test", withholdingTax: 10 },
  { userEmail: "nurse@example.test",  withholdingTax: 5 },
];
...
//   apps/<app>/src/pages/pme/reportTemplatePresets.ts
// We dynamically import them when the sibling product repo is checked out
// alongside the infra repo (default workspace layout). When unavailable the
// step is skipped with a warning rather than failing the whole seed.
//
// Body shape verified from apps/<app>/src/pages/pme/FormTemplateDetail.vue
// submit handler:
...
async function seedUserProfiles(userIds: Record<string, string>): Promise<void> {
  ...
  for (const user of USERS) {
    ...
    try {
      ...
      const msg = (err as Error).message;
      if (msg.includes("429")) {
        // Signing in as N users back-to-back can trip BETTER_AUTH_RATE_LIMIT.
        // Back off and retry once rather than skipping the profile PATCH.
        await new Promise((r) => setTimeout(r, 10_000));
        try {
          sessionCookie = "";
          await signIn(user.email, PASSWORD);
          ...
          console.warn(
            chalk.yellow(`  ⚠  ${user.email}: sign-in rate-limit retry failed — profile skipped`),
          );
          skipped++;
          continue;
        }
    ...
    const body: Record<string, unknown> = {
      ...
      // UserAvatar component (the product HR / messaging) renders picURL with
...
const SEED_PRIVACY_NOTICES: PrivacyNoticeTemplate[] = [
  {
    ...
    text:
      "Example Demo Clinic respects your privacy. By proceeding, you consent " +
  ...
  },
  {
    ...
    text:
      "Iginagalang ng Example Demo Clinic ang inyong privacy. Sa pagpapatuloy, " +
...
const LEGACY_SEED_ORG_NAMES = ["Example Demo Branch"];
...
const DM_PAIRS: Array<[string, string]> = [
  ["superadmin@example.test", "doctor@example.test"],
  ["superadmin@example.test", "admin@example.test"],
  ["superadmin@example.test", "nurse@example.test"],
  ["doctor@example.test",     "nurse@example.test"],
  ["doctor@example.test",     "pedia@example.test"],
  ["familymd@example.test",   "nurse@example.test"],
  ["nurse@example.test",      "cashier@example.test"],
  ["laboratory@example.test", "imaging@example.test"],
  ["doctor@example.test",     "laboratory@example.test"],
];
...
const GC_SPECS: GcSpec[] = [
  {
    title: "Leadership",
    creator: "superadmin@example.test",
    participants: ["superadmin@example.test", "admin@example.test", "doctor@example.test"],
  },
  {
    title: "Doctors",
    creator: "doctor@example.test",
    participants: ["doctor@example.test", "pedia@example.test", "familymd@example.test", "admin@example.test", "superadmin@example.test"],
  },
  {
    title: "Clinic Staff",
    creator: "admin@example.test",
    participants: [
      "superadmin@example.test", "admin@example.test", "doctor@example.test", "pedia@example.test", "familymd@example.test",
      "nurse@example.test", "cashier@example.test", "laboratory@example.test", "imaging@example.test",
    ],
  },
  {
    title: "Lab + Imaging",
    creator: "laboratory@example.test",
    participants: ["laboratory@example.test", "imaging@example.test", "doctor@example.test"],
  },
];
...
async function seedConversations(
  ...
  try {
    const list = await withUser("admin@example.test", () =>
    ...
    for (const conv of rows) {
      ...
      await withUser("admin@example.test", () =>
  ...
  try {
    const anns = await withUser("admin@example.test", () =>
...
async function main() {
  ...
  sessionCookie = "";
  await signIn("superadmin@example.test", PASSWORD);
  ...
  try {
    const probeConvs = await withUser("doctor@example.test", () =>
    ...
    if (!types.has("dm") || !types.has("gc")) {
      console.warn(chalk.yellow(`⚠ post-flight: doctor@example.test sees conversation types=[${[...types].join(", ")}] (expected dm + gc)`));
    } else {
      console.log(chalk.gray(`✓ doctor@example.test sees DM + GC conversations`));
    }
  } catch (err) {
