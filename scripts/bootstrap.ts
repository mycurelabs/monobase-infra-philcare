...
`;
    ...
    const path = 'rendered/external-secrets/argocd-github-app-externalsecret.yaml';
  ...
  async bootstrapInfrastructure() {
    ...
    try {
      ...
      // Server-side apply so re-bootstrap / tenant cutover onto a cluster that
      // ALREADY has argocd-managed Applications is idempotent. Client-side apply
      // fails on such objects ("resourceVersion must be specified for an update"
      // — no last-applied-configuration). --force-conflicts lets us take over
      // fields the ArgoCD controller manages. Not .quiet(): surface real output.
      await $`kubectl apply --server-side --force-conflicts -f - < ${new Response(rendered)}`;
...
function parseCliArgs(): BootstrapConfig {
  ...
  const clusterName = values['cluster-name'] || 'aws-main';
  // Per-cluster bootstrap override lives in values/ (tenant-owned), so a fork
  // never edits charts/argocd-bootstrap/values.yaml. Auto-loaded when present;
  // an explicit --bootstrap-values still wins.
  const perClusterBootstrap = `values/clusters/${clusterName}/argocd/bootstrap.yaml`;
  const config: BootstrapConfig = {
    ...
    bootstrapValues: values['bootstrap-values'] ?? (existsSync(perClusterBootstrap) ? perClusterBootstrap : undefined),
    clusterName,
  };
