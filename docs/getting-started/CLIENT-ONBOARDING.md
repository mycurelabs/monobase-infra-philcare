...
```bash
# On GitHub: Click "Fork" button on mycurelabs/monobase-infra-template

# Clone YOUR fork
git clone https://github.com/YOUR-ORG/monobase-infra-template.git
cd YOUR-FORK
```
...
shared `base.yaml` via `extends: base`.
...
template's `base.yaml`. To stage a namespace without deploying it yet, add
`disabled: "true"` to that file — it becomes parent-only until you remove the key.
...
git add values/deployments/myclient-production.yaml \
        values/deployments/myclient-staging.yaml
...
# 3. ApplicationSet (auto-discovers <client>-<env>.yaml files in values/deployments/)
...
kubectl get applications -n argocd | grep myclient-production

# You should see Applications created automatically:
# - myclient-production-namespace
# - myclient-production-security
# - myclient-production-postgresql
# - myclient-production-hapihub
# - myclient-production-<frontend>   # generic-app instances
# etc.
...
kubectl get pods -n myclient-production

# Check Gateway HTTPRoutes
kubectl get httproutes -n myclient-production
...
Edit `values/clusters/<cluster>/argocd/infrastructure.yaml`:
...
- Check SecretStore is created: `kubectl get secretstore -n myclient-production`
- Check IAM permissions (IRSA, Workload Identity)
- Check KMS secret exists and is accessible

### Pods Not Starting

- Check events: `kubectl describe pod <pod-name> -n myclient-production`
- Check logs: `kubectl logs <pod-name> -n myclient-production`
- Check resource quotas: `kubectl describe resourcequota -n myclient-production`

### Gateway Not Working

- Check Gateway status: `kubectl get gateway -n nginx-gateway-system`
- Check HTTPRoute status: `kubectl get httproute -n myclient-production`
