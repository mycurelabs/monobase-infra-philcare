...
# (limit_req) in values/clusters/<cluster>/argocd/infrastructure.yaml if needed.
```
...
# SnippetsPolicy in values/clusters/<cluster>/argocd/infrastructure.yaml). Also strips the
...
trivy image ghcr.io/mycurelabs/api:5.215.2

# 4. Sign images (optional)
cosign sign ghcr.io/mycurelabs/api:5.215.2

# 5. Verify signatures
cosign verify ghcr.io/mycurelabs/api:5.215.2
```
...
```bash
# On a managed cloud cluster (e.g. the aws-main EKS example, storageClass: gp3),
# volumes are encrypted at rest by the cloud provider's CSI driver — there is no
# Longhorn StorageClass to configure. Other providers use their own encrypted CSI
# StorageClass (Azure managed-premium, GCP pd-ssd, DO do-block-storage).
...
kubectl set image deployment/api \\
  api=ghcr.io/mycurelabs/api:5.215.3-patched \\
  -n myclient-prod
```
...
trivy image ghcr.io/mycurelabs/hapihub:11.20.118

# Scan Helm charts
trivy config charts/hapihub

# Scan Kubernetes manifests
trivy kubernetes --namespace <client>-production
...
- [SECURITY_COMPLIANCE.md](SECURITY_COMPLIANCE.md) - Compliance checklist (HIPAA, SOC 2, GDPR)
- [BACKUP_DR.md](../operations/BACKUP_DR.md) - Backup & DR procedures
