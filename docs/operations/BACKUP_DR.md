...
|------|-----------|-----------|---------|-----|-----|----------|
| **1** | Hourly | 72h | CSI volume snapshots (local) | 5 min | 1h | Quick rollback |
| **2** | Daily | 30d | S3 / object storage (off-cluster) | 1h | 24h | Recent recovery |
| **3** | Weekly | 90d | S3 Glacier (archive) | 4h | 1w | Compliance, DR |

> The tiers above protect **production data** (Kubernetes PVs / Velero).
...
GATEWAY_IP=$(kubectl get gateway nginx-shared-gateway -n nginx-gateway-system \\
...
| PostgreSQL | CSI volume snapshot | 5 min | Restore from snapshot |
