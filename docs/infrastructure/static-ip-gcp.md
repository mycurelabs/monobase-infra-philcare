...
Set the reserved IP by name on the Gateway's provisioned Service via `infrastructure.annotations` in `values/clusters/<cluster>/argocd/infrastructure.yaml` (see [static-ip-prerequisites.md](./static-ip-prerequisites.md)):
