...
Set the annotations on the Gateway's provisioned Service via `infrastructure.annotations` in `values/clusters/<cluster>/argocd/infrastructure.yaml` (see [static-ip-prerequisites.md](./static-ip-prerequisites.md)):
