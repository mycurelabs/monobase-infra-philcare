...
**What you deploy:** Cluster roots in `../values/clusters/<cluster>/terraform`
**Complements:** Application deployment in `../charts/` and `../values/deployments/`

## Quick Start

**Note:** You typically work with cluster roots under
`../values/clusters/<cluster>/terraform`, not these modules directly.

```bash
# Cluster roots live under values/clusters/<name>/terraform:
../values/clusters/
└── aws-main/
    ├── terraform/       # OpenTofu root (main.tf references modules from here)
    └── argocd/          # Cluster-wide infra values (ArgoCD-synced)

# Plan / apply a cluster via mise (wraps OpenTofu, backs up state first):
...
```
Single Cluster (provisioned via ../values/clusters/<cluster>/terraform)
...
An example cluster root is provided under `values/clusters/`:

- **`../values/clusters/aws-main/terraform/`** - Reference OpenTofu root
  (swap the module in `main.tf` for your target provider)
...
- ✅ **values/clusters/aws-main/terraform** - Reference OpenTofu root
  (point `main.tf` at any of the six modules above)
